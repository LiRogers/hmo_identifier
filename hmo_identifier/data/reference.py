...
def query_ons(parent: str = "E12000007", code_filter:str = "",
              inc_uri: bool = False,
              inc_parent: bool = False,
              inc_geom: bool = False) -> Union[pd.DataFrame,gpd.GeoDataFrame]:
    """
...
# %% London Boroughs
def london_boroughs(borough: str = None,
                    inc_geom: bool =False) -> Union[pd.DataFrame, gpd.GeoDataFrame]:
    """
    ...
    """
    df = query_ons(parent="E12000007", code_filter="E09",
                   inc_geom=inc_geom)
    df = df.rename(columns = {"code": "ladcd",
                              "name": "ladnm"})
...
# %% London Wards
def london_wards(borough:str = None, combined: bool = False,
                 inc_geom: bool = False) -> Union[pd.DataFrame, gpd.GeoDataFrame]:
    """
    ...
    inc_geom : bool, optional
        Include geometry of the wards. Will return a GeoDataFrame if set to True.
        The default is False.
    ...
    df = pd.concat([
        (query_ons(parent = row.ladcd, code_filter = code_filter,
                   inc_geom=inc_geom)
         .assign(ladcd = row.ladcd,
                 ladnm = row.ladnm)) for i, row in boroughs.iterrows()])

    if combined:
        df = df.rename(columns={"code": "wardcmcd",
                                  "name": "wardcmnm"})
    else:
        df = df.rename(columns={"code": "wardcd",
                                  "name": "wardnm"})
...
def london_output_areas(borough: str = None,
                        inc_geom:bool = False) -> Union[pd.DataFrame, gpd.GeoDataFrame]:
    """
    ...
    inc_geom : bool, optional
        Include geometry of the Output Areas. Will return a GeoDataFrame if set to True.
        The default is False.
    ...
    for code in borough_codes:
        oa = query_ons(parent=code, code_filter="E00",
                              inc_parent=True, inc_geom=inc_geom)
        oas = oas.append(oa, sort=True)
    oas = oas.rename(columns={"code": "oacd",
                              "name": "oanm",
                              "parent": "lsoacd"})    
    lsoas = query_ons(code_filter="E01", inc_parent=True)
    lsoas = lsoas.rename(columns={"code": "lsoacd",
                              "name": "lsoanm",
                              "parent": "msoacd"})
    msoas = query_ons(code_filter="E02")
    msoas = (msoas
             .assign(ladnm = msoas.name.str.extract("^([A-Za-z ]+) [0-9]+",
                                                    expand=False))
             .rename(columns = {"code": "msoacd",
                                "name": "msoanm"}))
    output_areas = (oas
                    .merge(lsoas)
                    .merge(msoas)
